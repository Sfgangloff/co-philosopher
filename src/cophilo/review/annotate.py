...
_BIBCHECK_TAG = {
    ...
    "missing_citation": "CITE-NEEDED",
}


def _bibcheck_block(checks, syntax: CommentSyntax) -> list[str]:
    """Render the bibliography-check verdicts as a trailing section.

    Closes the §3 loop: at draft time the bibliography is judged for tier;
    at review time the *draft's claims* are checked against that same
    judgement. Verdicts and the works that justify them are surfaced
    together so the reader can see what the synthesis already vouched for."""
    if not checks:
        return []
    rule = "─" * 60
    out = [syntax.line(rule), syntax.line("BIBLIOGRAPHY CHECK")]
    for c in checks:
        tag = _BIBCHECK_TAG.get(c.verdict, c.verdict.upper())
        line_hint = f" (line {c.evidence_line})" if c.evidence_line else ""
        cites = f"  [cite: {', '.join(c.cite_suggestions)}]" if c.cite_suggestions else ""
        body = f"[{tag}] {c.claim.strip()}{line_hint} — {c.evidence.strip()}{cites}"
        for p in _wrap("", body):
            out.append(syntax.line(p))
    out.append(syntax.line(rule))
    return out
...
def sidecar_markdown(
    ...
    if coverage:
        ...
        out.append("")
    checks = getattr(review, "bibliography_check", None) or []
    if checks:
        out += ["## Bibliography check", ""]
        for c in checks:
            tag = _BIBCHECK_TAG.get(c.verdict, c.verdict.upper())
            line_hint = f" (line {c.evidence_line})" if c.evidence_line else ""
            out.append(f"- **[{tag}]** {c.claim.strip()}{line_hint}")
            if c.evidence.strip():
                out.append(f"  {c.evidence.strip()}")
            if c.cite_suggestions:
                out.append(f"  *cite:* {', '.join(c.cite_suggestions)}")
        out.append("")
...
def annotate(
    ...
    *,
    ...
    checks = getattr(review, "bibliography_check", None) or []
    if checks:
        out.extend(_bibcheck_block(checks, syntax))
